...
from .pilot2_injection_executor_v025 import (
    RECORD_RUN_ID,
...
RUNNER_CONFIG_PATH = "config/pilot2_injection_runner_v0.2.5.yaml"
IMPLEMENTATION_FREEZE_PATH = "protocol/PILOT2_INJECTION_RUNNER_FREEZE_v0.2.5.json"
READINESS_FREEZE_PATH = "protocol/PILOT2_INJECTION_EXECUTION_READINESS_FREEZE_v0.2.5.json"
EXECUTION_FREEZE_PATH = "protocol/PILOT2_INJECTION_EXECUTION_FREEZE_v0.2.5.json"
...
IMPLEMENTATION_PATHS = (
    "src/pulsar_pilot/pilot2_injection_runner_v025.py",
    "src/pulsar_pilot/pilot2_injection_executor_v025.py",
...
IMPLEMENTATION_FREEZE_FROZEN_PATHS = (
    "config/pilot2_injection_runner_v0.2.5.yaml",
    "protocol/PILOT2_INJECTION_RUNNER_PROTOCOL_v0.2.5.md",
    "results/pilot2/injection_v025_audit_finding_closure.json",
    "results/pilot2/injection_validation_manifest_v0.2.5.json",
    "src/pulsar_pilot/pilot2_injection_executor_v025.py",
    "src/pulsar_pilot/pilot2_injection_runner_v025.py",
    "tests/test_pilot2_injection_runner_v025.py",
)
READINESS_FREEZE_FROZEN_PATHS = (
    "docs/PILOT2_INJECTION_EXECUTION_READINESS_v0.2.5.md",
    "protocol/PILOT2_INJECTION_RUNNER_FREEZE_v0.2.5.json",
    "results/pilot2/injection_execution_readiness_v0.2.5.json",
    "results/pilot2/injection_runner_zero_science_validation_v0.2.5.json",
    "results/pilot2/injection_v025_audit_finding_closure.json",
    "results/pilot2/injection_validation_manifest_v0.2.5.json",
    "src/pulsar_pilot/pilot2_injection_executor_v025.py",
    "src/pulsar_pilot/pilot2_injection_runner_v025.py",
...
EXECUTION_FREEZE_FROZEN_PATHS = (
    "config/pilot2_injection_runner_v0.2.5.yaml",
    "docs/PILOT2_INJECTION_EXECUTION_READINESS_v0.2.5.md",
    "docs/audits/DEDICATED_SOL_AUDIT_PILOT2_v0.2.5_2026-08-10.md",
    "protocol/DEDICATED_SOL_AUDIT_RECORD_PILOT2_v0.2.5.json",
    "protocol/PILOT2_INJECTION_EXECUTION_READINESS_FREEZE_v0.2.5.json",
    "protocol/PILOT2_INJECTION_REMEDIATION_FREEZE_v0.2.3.json",
    "protocol/PILOT2_INJECTION_RUNNER_FREEZE_v0.2.5.json",
    "protocol/PILOT2_INJECTION_RUNNER_PROTOCOL_v0.2.5.md",
    "results/pilot2/dedicated_sol_audit_disposition_v0.2.5.json",
    "results/pilot2/dedicated_sol_audit_primary_verification_v0.2.5.json",
    "results/pilot2/injection_execution_readiness_v0.2.5.json",
    "results/pilot2/injection_runner_zero_science_validation_v0.2.5.json",
    "results/pilot2/injection_v025_audit_finding_closure.json",
    "results/pilot2/injection_validation_manifest_v0.2.5.json",
...
def _verify_exact_repository_hashes(
    ...
    if not isinstance(frozen, dict):
        return [f"v0.2.5 {label} frozen-hash contract is not an object"]
    ...
    if actual != expected:
        ...
        return [
            (
                f"v0.2.5 {label} frozen-hash path set is not exact: "
    ...
    for relative in expected_paths:
        ...
        if relative_path.is_absolute() or ".." in relative_path.parts:
            failures.append(f"v0.2.5 {label} unsafe hash path: {relative}")
            continue
        item = root / relative_path
        try:
            resolved_item = item.resolve(strict=True)
        except OSError:
            failures.append(f"v0.2.5 {label} hash target is absent: {relative}")
            continue
        if not resolved_item.is_relative_to(resolved_root) or item.is_symlink():
            failures.append(f"v0.2.5 {label} hash path escapes repository: {relative}")
            continue
        expected_hash = frozen[relative]
        if not isinstance(expected_hash, str) or hash_file(item, "sha256") != expected_hash:
            failures.append(f"v0.2.5 {label} hash mismatch: {relative}")
    return failures
...
def _validate_input_binding(binding: object) -> list[str]:
    if not isinstance(binding, dict) or set(binding) != INPUT_BINDING_KEYS:
        return ["input binding schema differs"]
    failures: list[str] = []
    expected = {
        "target": "B1937+21",
        "active_toas": 660,
    ...
    for key, value in expected.items():
        if binding.get(key) != value:
            failures.append(f"input binding differs: {key}")
    if not _finite_number(binding.get("reference_epoch_mjd_tdb")):
        failures.append("input binding reference epoch is invalid")
    grid = binding.get("frequency_grid")
    if not isinstance(grid, dict) or set(grid) != FREQUENCY_GRID_KEYS:
        failures.append("input binding frequency-grid schema differs")
    else:
        if grid.get("algorithm") != "linear_frequency_independent_bin_oversampling":
            failures.append("input binding frequency-grid algorithm differs")
        if (
            grid.get("oversampling") != 5
            or type(grid.get("frequency_count")) is not int
            or grid["frequency_count"] < 2
        ):
            failures.append("input binding frequency-grid count or oversampling differs")
        for key in FREQUENCY_GRID_KEYS - {"algorithm", "oversampling", "frequency_count"}:
            if not _finite_number(grid.get(key)):
                failures.append(f"input binding frequency-grid value is invalid: {key}")
        if grid.get("minimum_period_days") != 30.0:
            failures.append("input binding minimum search period differs")
        if grid.get("maximum_period_days") != 2000.0:
            failures.append("input binding maximum search period differs")
        if grid.get("minimum_frequency_per_day") != 1.0 / 2000.0:
            failures.append("input binding minimum search frequency differs")
        if grid.get("maximum_frequency_per_day") != 1.0 / 30.0:
            failures.append("input binding maximum search frequency differs")
        if _finite_number(grid.get("span_days")) and _finite_number(
            ...
            if not math.isclose(
                ...
                failures.append("input binding frequency step is inconsistent")
    return failures
...
def validate_complete_case_record(
    ...
    failures.extend(_validate_input_binding(record.get("input_binding")))
...
def _verify_freeze(
    ...
    path = root / path_name
    if not path.is_file():
        return {"status": "locked", "failures": [f"v0.2.5 {label} freeze is absent"]}
    resolved_root = root.resolve()
    if path.is_symlink() or not path.resolve(strict=True).is_relative_to(resolved_root):
        return {
            "status": "fail",
            "failures": [f"v0.2.5 {label} freeze path escapes repository"],
        }
    try:
        ...
        return {
            "status": "fail",
            "failures": [f"v0.2.5 {label} freeze is unreadable: {error}"],
        }
    if not isinstance(freeze, dict):
        return {
            "status": "fail",
            "failures": [f"v0.2.5 {label} freeze is not an object"],
        }
    failures: list[str] = []
    if freeze.get("status") != expected_status:
        failures.append(f"v0.2.5 {label} freeze status is invalid")
    if freeze.get("execution_authorized") is not False:
        failures.append(f"v0.2.5 {label} freeze authorizes execution")
    if freeze.get("science_cases_executed_during_validation") != 0:
        failures.append(f"v0.2.5 {label} validation was not zero-science")
    for key in (
        ...
        if freeze.get(key) is not False:
            failures.append(f"v0.2.5 {label} boundary invalid: {key}")
    inventory = _inventory()
    if freeze.get("inventory_sha256") != inventory["inventory_sha256"]:
        failures.append(f"v0.2.5 {label} inventory mismatch")
    if freeze.get("implementation_sha256") != implementation_sha256():
        failures.append(f"v0.2.5 {label} implementation mismatch")
    failures.extend(
...
def verify_readiness_freeze() -> dict[str, Any]:
    ...
    if freeze.get("execution_readiness") != "pass":
        failures.append("v0.2.5 readiness semantic is not pass")
...
def verify_predecessor_bindings(data_root: Path) -> dict[str, Any]:
    ...
    repository_keys = {
        ...
        "v0.2.2_crash_audit",
        "v0.2.4_sol_audit_record",
        "v0.2.4_sol_audit_disposition",
    }
    ...
    v022_ledger = json.loads((data_root / bindings["v0.2.2_ledger"]["logical_path"]).read_text())
...
def verify_repository_execution_authorization() -> dict[str, Any]:
    ...
    if not path.is_file():
        return {
            ...
            "failures": failures + ["Separate v0.2.5 execution freeze is absent"],
    ...
    if path.is_symlink() or not path.resolve(strict=True).is_relative_to(resolved_root):
        return {
            "status": "fail",
            "failures": failures + ["v0.2.5 execution freeze path escapes repository"],
            "implementation": implementation,
            "readiness": readiness,
            "predecessors_loaded": False,
        }
    try:
        ...
        return {
            "status": "fail",
            "failures": failures + [f"v0.2.5 execution freeze is unreadable: {error}"],
            "implementation": implementation,
            "readiness": readiness,
            "predecessors_loaded": False,
        }
    if not isinstance(freeze, dict):
        return {
            "status": "fail",
            "failures": failures + ["v0.2.5 execution freeze is not an object"],
            "implementation": implementation,
            "readiness": readiness,
            "predecessors_loaded": False,
        }
    if freeze.get("schema_version") != 1:
        failures.append("v0.2.5 execution freeze schema version is invalid")
    if freeze.get("freeze_id") != "pilot2-b1937-injection-execution-freeze-v0.2.5":
        failures.append("v0.2.5 execution freeze ID is invalid")
    if freeze.get("status") != "frozen_before_first_v0.2.5_injection_case":
        failures.append("v0.2.5 execution freeze status is invalid")
    if (
        not isinstance(freeze.get("user_authorization"), str)
        or not freeze["user_authorization"].strip()
    ):
        failures.append("v0.2.5 execution user authorization is absent")
    if freeze.get("execution_authorized") is not True:
        failures.append("v0.2.5 execution authorization is absent")
    if freeze.get("authorized_case_counts") != EXPECTED_AUTHORIZED_CASE_COUNTS:
        failures.append("v0.2.5 authorized case counts are invalid")
    if freeze.get("authorized_primary_fits") != 688:
        failures.append("v0.2.5 authorized primary-fit count is invalid")
    if freeze.get("no_reroll") is not True:
        failures.append("v0.2.5 no-reroll contract is absent")
    if freeze.get("threshold_retuning_authorized") is not False:
        failures.append("v0.2.5 threshold-retuning boundary is invalid")
    if freeze.get("v0.2.1_injection_acceptance_reuse_authorized") is not False:
        failures.append("v0.2.5 predecessor acceptance-reuse boundary is invalid")
    for key in (
        "observed_residual_access_authorized",
        "observed_periodic_search_authorized",
        "promotion_grade_authorized",
        ...
        if freeze.get(key) is not False:
            failures.append(f"v0.2.5 execution boundary invalid: {key}")
    inventory = _inventory()
    if freeze.get("inventory_sha256") != inventory["inventory_sha256"]:
        failures.append("v0.2.5 execution inventory mismatch")
    if freeze.get("implementation_sha256") != implementation_sha256():
        failures.append("v0.2.5 execution implementation mismatch")
    if freeze.get("implementation_freeze_sha256") != implementation.get("freeze_sha256"):
        failures.append("v0.2.5 execution implementation-freeze mismatch")
    if freeze.get("readiness_freeze_sha256") != readiness.get("freeze_sha256"):
        failures.append("v0.2.5 execution readiness-freeze mismatch")
    _, _, runner = _configs()
    ...
    if freeze.get("predecessor_sha256") != expected_predecessors:
        failures.append("v0.2.5 execution predecessor contract mismatch")
    threshold_sha = expected_predecessors["threshold_lock"]
    if freeze.get("threshold_lock_sha256") != threshold_sha:
        failures.append("v0.2.5 execution threshold-lock mismatch")
    ...
    if freeze.get("supervision_policy") != expected_supervision:
        failures.append("v0.2.5 execution supervision policy is invalid")
    if freeze.get("stop_rule") != (
        "any_gate_failure_stops_before_or_during_execution_without_reroll_retune_or_promotion"
    ):
        failures.append("v0.2.5 execution stop rule is invalid")
    failures.extend(
        _verify_exact_repository_hashes(
            root,
            freeze.get("frozen_sha256"),
...
class V025Ledger(CalibrationLedger):
    def __init__(self, data_root: Path, inventory_sha256: str, implementation_hash: str):
        ...
        self.path = data_root / "run_records/pilot2/calibration-v0.2.5-ledger.json"
        self.health_path = data_root / "run_records/pilot2/calibration-v0.2.5-health.json"
        self.dashboard_path = data_root / "derived/pilot2/calibration-v0.2.5-dashboard/index.html"
    ...
    def begin_stage(self, stage: str) -> dict[str, Any]:
        if stage != STAGE:
            raise ValueError(f"Unknown v0.2.5 runner stage: {stage}")
        state = self.load()
        if state["hard_stop"] is not None:
            raise RuntimeError("v0.2.5 ledger contains a hard stop")
        if state["stage_status"][STAGE] == "pass":
            raise RuntimeError("v0.2.5 injection stage is already complete")
        if state["active_stage"] not in {None, STAGE}:
            raise RuntimeError("A different v0.2.5 stage is active")
    ...
    def health(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        health.update(
            ...
            manual_refresh_command=(
                ...
                "pulsar_pilot.pilot2_injection_runner_v025 health --data-root <PATH>"
            ),
        )
        return health
    ...
    def begin_attempt(
        ...
        if state.get("active_attempt") is not None:
            raise RuntimeError("A prior v0.2.5 attempt requires recovery disposition")
        attempt = {
    ...
    def clear_attempt(self, case_id: str, disposition: str) -> None:
        ...
        if active is None or active.get("case_id") != case_id:
            raise RuntimeError("Active v0.2.5 attempt does not match completed case")
...
def prepare_context(data_root: Path, cases: list[dict[str, Any]]) -> InjectionContext:
    log_path = data_root / "run_records/pilot2/injection-v0.2.5-setup.log"
    ...
    for key, value in expected.items():
        if binding[key] != value:
            raise RuntimeError(f"v0.2.5 injection setup mismatch: {key}")
...
def _record_path(data_root: Path, sequence: int, case: dict[str, Any]) -> Path:
    return (
        data_root
        / "derived/pilot2/calibration-v0.2.5/injections"
        / str(case["family"])
...
def recover_active_attempt(
    ...
    try:
        ...
        raise RuntimeError(
            "A consumed v0.2.5 attempt lacks a verified adoptable artifact"
        ) from error
...
def execute(data_root: Path) -> dict[str, Any]:
    ...
    if gate["status"] != "pass":
        raise RuntimeError(f"v0.2.5 injection execution is locked: {gate['failures']}")
    require_initialized_data_root(data_root)
    base, remediation, runner = _configs()
    inventory = _inventory()
    cases = inventory["cases"]
    audit_ids = set(inventory["solver_audit_case_ids"])
    ledger = V025Ledger(data_root, inventory["inventory_sha256"], implementation_sha256())
    result_path = data_root / runner["paths"]["result"]
    current_case_id: str | None = None

    try:
        if ledger.verify_artifacts()["status"] != "pass":
            raise RuntimeError("v0.2.5 recorded artifact integrity failure")
        state = ledger.load()
        if state["stage_status"][STAGE] == "pass":
            return {
        ...
        if state["active_stage"] is None:
            ...
            raise RuntimeError("v0.2.5 ledger is not in a resumable stage state")
        ...
        def enforce_runtime_cap() -> None:
            ...
            if elapsed > limit:
                raise RuntimeError("v0.2.5 injection stage exceeded the six-hour cap")
        ...
        def load_or_execute(
            ...
            if current_case_id in ledger.load()["completed_cases"]:
                ...
                return record, False
            ledger.begin_attempt(sequence, case, execution_binding, path)
        ...
        def process_nonannual(family: str) -> None:
            for sequence, case in (
                ...
                print(
                    f"PILOT2R5_INJECTION_PROGRESS {len(records_by_id)}/344",
                    flush=True,
                )
                enforce_runtime_cap()
            ...
            for period in annual_periods:
                ...
                for record in group_records:
                    if (
                        ...
                        raise RuntimeError("Resumed v0.2.5 annual eligibility differs")
        ...
        if hash_file(lock_path, "sha256") != lock_sha:
            raise RuntimeError("Predecessor threshold lock changed during v0.2.5 execution")
...
def zero_case_dry_run(data_root: Path) -> dict[str, Any]:
    ...
    allowed_lock_failures = {
        "v0.2.5 readiness freeze is absent",
        "Separate v0.2.5 execution freeze is absent",
    }
    criteria = {
        ...
        "execution_gate_fail_closed": (
            ...
            and "Separate v0.2.5 execution freeze is absent" in gate["failures"]
    ...
    return {
        ...
        "operation": "v0.2.5_runner_zero_science_integrity_dry_run",
        "criteria": criteria,
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="pilot2-injection-runner-v025")
    parser.add_argument(

...
from pulsar_pilot import pilot2_injection_runner_v026 as runner_module
...
from pulsar_pilot.pilot2_injection_runner_v026 import (
...
def _ledger(tmp_path: Path) -> V026Ledger:
...
def test_v026_reuses_never_executed_v023_inventory_without_seed_churn() -> None:
    inventory = build_v023_inventory()
...
def test_readiness_semantic_must_explicitly_pass(
    ...
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.READINESS_FREEZE_PATH",
        str(freeze_path),
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026._verify_freeze",
        lambda *_: {
            "status": "pass",
            "failures": [],
            "freeze_sha256": "r" * 64,
...
def test_malformed_present_execution_freeze_stops_before_predecessor_loading(
    ...
    freeze_path = tmp_path / "protocol/PILOT2_INJECTION_EXECUTION_FREEZE_v0.2.6.json"
    freeze_path.parent.mkdir(parents=True)
    freeze_path.write_text("{not-json")
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.repository_root",
        lambda: tmp_path,
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.verify_implementation_freeze",
        lambda: {"status": "pass", "failures": [], "freeze_sha256": "i" * 64},
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.verify_readiness_freeze",
        lambda: {"status": "pass", "failures": [], "freeze_sha256": "r" * 64},
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.verify_predecessor_bindings",
...
def test_execution_freeze_full_contract_is_required_and_hash_verified(
    ...
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.implementation_sha256",
        lambda: "m" * 64,
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026._configs",
        lambda: ({}, {}, runner),
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.EXECUTION_FREEZE_FROZEN_PATHS",
        (frozen_path,),
    )
    freeze = {
        "schema_version": 1,
        "freeze_id": "pilot2-b1937-injection-execution-freeze-v0.2.6",
        "status": "frozen_before_first_v0.2.6_injection_case",
...
def test_symlinked_hash_target_cannot_escape_repository(tmp_path: Path) -> None:
    ...
    assert failures == ["v0.2.6 execution hash path escapes repository: link.json"]
...
def test_case_directories_are_durably_precreated(tmp_path: Path) -> None:
    ...
    case_root = tmp_path / "derived/pilot2/calibration-v0.2.6/injections"
...
def test_keyboard_interrupt_leaves_durable_consumed_attempt_for_recovery(
    ...
    lock_path.write_text("{}")
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.verify_execution_gate",
        lambda *_: _fake_gate(),
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.prepare_context",
        lambda *_: _context(),
    )
    monkeypatch.setattr(
        "pulsar_pilot.pilot2_injection_runner_v026.execute_injection_case",
...
def test_post_begin_failure_is_terminalized_by_execute(
    ...
    with pytest.raises(FileNotFoundError):
        execute(ledger.data_root)
    state = ledger.load()
    result_path = ledger.data_root / "run_records/pilot2/injection-evaluation-v0.2.6.json"
    result = json.loads(result_path.read_text())
    assert state["stage_status"][STAGE] == "fail"
    assert state["hard_stop"] is not None

...
RUNNER_CONFIG_PATH = "config/pilot2_injection_runner_v0.2.4.yaml"
IMPLEMENTATION_FREEZE_PATH = "protocol/PILOT2_INJECTION_RUNNER_FREEZE_v0.2.4.json"
READINESS_FREEZE_PATH = "protocol/PILOT2_INJECTION_EXECUTION_READINESS_FREEZE_v0.2.4.json"
EXECUTION_FREEZE_PATH = "protocol/PILOT2_INJECTION_EXECUTION_FREEZE_v0.2.4.json"
STAGE = "injection_remediation_evaluation"
RUN_ID = "pilot2-b1937-injection-remediation-v0.2.4"
...
IMPLEMENTATION_PATHS = (
    "src/pulsar_pilot/pilot2_injection_runner_v024.py",
...
def _verify_freeze(path_name: str, expected_status: str, label: str) -> dict[str, Any]:
    ...
    path = root / path_name
    if not path.is_file():
        return {"status": "locked", "failures": [f"v0.2.4 {label} freeze is absent"]}
    try:
        freeze = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        return {
            "status": "fail",
            "failures": [f"v0.2.4 {label} freeze is unreadable: {error}"],
        }
    if not isinstance(freeze, dict):
        return {
            "status": "fail",
            "failures": [f"v0.2.4 {label} freeze is not an object"],
        }
    failures: list[str] = []
    if freeze.get("status") != expected_status:
        failures.append(f"v0.2.4 {label} freeze status is invalid")
    if freeze.get("execution_authorized") is not False:
        failures.append(f"v0.2.4 {label} freeze authorizes execution")
    if freeze.get("science_cases_executed_during_validation") != 0:
        failures.append(f"v0.2.4 {label} validation was not zero-science")
    for key in (
        ...
        if freeze.get(key) is not False:
            failures.append(f"v0.2.4 {label} boundary invalid: {key}")
    inventory = _inventory()
    if freeze.get("inventory_sha256") != inventory["inventory_sha256"]:
        failures.append(f"v0.2.4 {label} inventory mismatch")
    if freeze.get("implementation_sha256") != implementation_sha256():
        failures.append(f"v0.2.4 {label} implementation mismatch")
    frozen = freeze.get("frozen_sha256")
    if not isinstance(frozen, dict) or not frozen:
        failures.append(f"v0.2.4 {label} frozen-hash contract is absent")
    else:
        for relative, expected in frozen.items():
            item = root / relative
            if not item.is_file() or hash_file(item, "sha256") != expected:
                failures.append(f"v0.2.4 {label} hash mismatch: {relative}")
    return {
...
def verify_readiness_freeze() -> dict[str, Any]:
    ...
    if freeze.get("execution_readiness") != "pass":
        failures.append("v0.2.4 readiness semantic is not pass")
...
def verify_predecessor_bindings(data_root: Path) -> dict[str, Any]:
    ...
    repository_keys = {
        ...
        "v0.2.2_crash_audit",
        "v0.2.3_sol_audit_record",
        "v0.2.3_sol_audit_disposition",
    }
...
def verify_repository_execution_authorization(
    ...
    path = execution_freeze_path or root / EXECUTION_FREEZE_PATH
    if not path.is_file():
        return {
            "status": "locked" if not failures else "fail",
            "failures": failures + ["Separate v0.2.4 execution freeze is absent"],
    ...
    try:
        ...
        return {
            "status": "fail",
            "failures": failures + [f"v0.2.4 execution freeze is unreadable: {error}"],
            "implementation": implementation,
            "readiness": readiness,
            "predecessors_loaded": False,
        }
    if not isinstance(freeze, dict):
        return {
            "status": "fail",
            "failures": failures + ["v0.2.4 execution freeze is not an object"],
            "implementation": implementation,
            "readiness": readiness,
            "predecessors_loaded": False,
        }
    if freeze.get("schema_version") != 1:
        failures.append("v0.2.4 execution freeze schema version is invalid")
    if freeze.get("freeze_id") != "pilot2-b1937-injection-execution-freeze-v0.2.4":
        failures.append("v0.2.4 execution freeze ID is invalid")
    if freeze.get("status") != "frozen_before_first_v0.2.4_injection_case":
        failures.append("v0.2.4 execution freeze status is invalid")
    if (
        not isinstance(freeze.get("user_authorization"), str)
        or not freeze["user_authorization"].strip()
    ):
        failures.append("v0.2.4 execution user authorization is absent")
    if freeze.get("execution_authorized") is not True:
        failures.append("v0.2.4 execution authorization is absent")
    if freeze.get("authorized_case_counts") != EXPECTED_AUTHORIZED_CASE_COUNTS:
        failures.append("v0.2.4 authorized case counts are invalid")
    if freeze.get("authorized_primary_fits") != 688:
        failures.append("v0.2.4 authorized primary-fit count is invalid")
    if freeze.get("no_reroll") is not True:
        failures.append("v0.2.4 no-reroll contract is absent")
    if freeze.get("threshold_retuning_authorized") is not False:
        failures.append("v0.2.4 threshold-retuning boundary is invalid")
    if freeze.get("v0.2.1_injection_acceptance_reuse_authorized") is not False:
        failures.append("v0.2.4 predecessor acceptance-reuse boundary is invalid")
    for key in (
        "observed_residual_access_authorized",
        "observed_periodic_search_authorized",
        "promotion_grade_authorized",
        ...
        if freeze.get(key) is not False:
            failures.append(f"v0.2.4 execution boundary invalid: {key}")
    inventory = _inventory()
    if freeze.get("inventory_sha256") != inventory["inventory_sha256"]:
        failures.append("v0.2.4 execution inventory mismatch")
    if freeze.get("implementation_sha256") != implementation_sha256():
        failures.append("v0.2.4 execution implementation mismatch")
    if freeze.get("implementation_freeze_sha256") != implementation.get("freeze_sha256"):
        failures.append("v0.2.4 execution implementation-freeze mismatch")
    if freeze.get("readiness_freeze_sha256") != readiness.get("freeze_sha256"):
        failures.append("v0.2.4 execution readiness-freeze mismatch")
    _, _, runner = _configs()
    ...
    if freeze.get("predecessor_sha256") != expected_predecessors:
        failures.append("v0.2.4 execution predecessor contract mismatch")
    threshold_sha = expected_predecessors["threshold_lock"]
    if freeze.get("threshold_lock_sha256") != threshold_sha:
        failures.append("v0.2.4 execution threshold-lock mismatch")
    ...
    if freeze.get("supervision_policy") != expected_supervision:
        failures.append("v0.2.4 execution supervision policy is invalid")
    if freeze.get("stop_rule") != (
        "any_gate_failure_stops_before_or_during_execution_without_reroll_retune_or_promotion"
    ):
        failures.append("v0.2.4 execution stop rule is invalid")
    frozen = freeze.get("frozen_sha256")
    if not isinstance(frozen, dict) or not frozen:
        failures.append("v0.2.4 execution frozen-hash contract is absent")
    else:
        for relative, expected in frozen.items():
            item = root / relative
            if not item.is_file() or hash_file(item, "sha256") != expected:
                failures.append(f"v0.2.4 execution hash mismatch: {relative}")
    return {
        "status": "pass" if not failures else "fail",
        "failures": failures,
        "implementation": implementation,
...
class V024Ledger(CalibrationLedger):
    def __init__(self, data_root: Path, inventory_sha256: str, implementation_hash: str):
        ...
        self.path = data_root / "run_records/pilot2/calibration-v0.2.4-ledger.json"
        self.health_path = data_root / "run_records/pilot2/calibration-v0.2.4-health.json"
        self.dashboard_path = data_root / "derived/pilot2/calibration-v0.2.4-dashboard/index.html"
    ...
    def begin_stage(self, stage: str) -> dict[str, Any]:
        if stage != STAGE:
            raise ValueError(f"Unknown v0.2.4 runner stage: {stage}")
        state = self.load()
        if state["hard_stop"] is not None:
            raise RuntimeError("v0.2.4 ledger contains a hard stop")
        if state["stage_status"][STAGE] == "pass":
            raise RuntimeError("v0.2.4 injection stage is already complete")
        if state["active_stage"] not in {None, STAGE}:
            raise RuntimeError("A different v0.2.4 stage is active")
    ...
    def health(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        health.update(
            ...
            manual_refresh_command=(
                ...
                "pulsar_pilot.pilot2_injection_runner_v024 health --data-root <PATH>"
            ),
        )
        return health
    ...
    def begin_attempt(
        ...
        if state.get("active_attempt") is not None:
            raise RuntimeError("A prior v0.2.4 attempt requires recovery disposition")
        attempt = {
    ...
    def clear_attempt(self, case_id: str, disposition: str) -> None:
        ...
        if active is None or active.get("case_id") != case_id:
            raise RuntimeError("Active v0.2.4 attempt does not match completed case")
...
def prepare_context(data_root: Path, cases: list[dict[str, Any]]) -> InjectionContext:
    log_path = data_root / "run_records/pilot2/injection-v0.2.4-setup.log"
    ...
    for key, value in expected.items():
        if binding[key] != value:
            raise RuntimeError(f"v0.2.4 injection setup mismatch: {key}")
...
def _record_path(data_root: Path, sequence: int, case: dict[str, Any]) -> Path:
    return (
        data_root
        / "derived/pilot2/calibration-v0.2.4/injections"
        / str(case["family"])
...
def _verified_attempt_artifact(
    ...
    if record.get("case") != case:
        raise RuntimeError("Interrupted attempt artifact case differs")
    if record.get("execution_binding_sha256") != execution_binding:
        raise RuntimeError("Interrupted attempt artifact binding differs")
    return path, record
...
def recover_active_attempt(
    ...
    if case is None:
        ...
        raise error
    try:
        path, record = _verified_attempt_artifact(ledger, attempt, case, execution_binding)
        ...
        raise RuntimeError(
            "A consumed v0.2.4 attempt lacks a verified adoptable artifact"
        ) from error
...
def execute(data_root: Path) -> dict[str, Any]:
    ...
    if gate["status"] != "pass":
        raise RuntimeError(f"v0.2.4 injection execution is locked: {gate['failures']}")
    require_initialized_data_root(data_root)
    base, remediation, runner = _configs()
    inventory = _inventory()
    cases = inventory["cases"]
    audit_ids = set(inventory["solver_audit_case_ids"])
    ledger = V024Ledger(data_root, inventory["inventory_sha256"], implementation_sha256())
    result_path = data_root / runner["paths"]["result"]
    current_case_id: str | None = None

    try:
        if ledger.verify_artifacts()["status"] != "pass":
            raise RuntimeError("v0.2.4 recorded artifact integrity failure")
        state = ledger.load()
        if state["stage_status"][STAGE] == "pass":
            return {
        ...
        if state["active_stage"] is None:
            ...
            raise RuntimeError("v0.2.4 ledger is not in a resumable stage state")
        ...
        recover_active_attempt(ledger, cases, execution_binding, result_path)
        ...
        def enforce_runtime_cap() -> None:
            ...
            if elapsed > limit:
                raise RuntimeError("v0.2.4 injection stage exceeded the six-hour cap")
        ...
        def load_or_execute(
            ...
            if current_case_id in ledger.load()["completed_cases"]:
                ...
                if (
                    ...
                    or record.get("execution_binding_sha256") != execution_binding
                ):
                    raise RuntimeError("Resumed v0.2.4 case binding differs")
                return record, False
        ...
        def process_nonannual(family: str) -> None:
            for sequence, case in (
                ...
                print(
                    f"PILOT2R4_INJECTION_PROGRESS {len(records_by_id)}/344",
                    flush=True,
                )
                enforce_runtime_cap()
            ...
            for period in annual_periods:
                ...
                for record in group_records:
                    if (
                        ...
                        raise RuntimeError("Resumed v0.2.4 annual eligibility differs")
        ...
        if hash_file(lock_path, "sha256") != lock_sha:
            raise RuntimeError("Predecessor threshold lock changed during v0.2.4 execution")
...
def zero_case_dry_run(data_root: Path) -> dict[str, Any]:
    ...
    allowed_lock_failures = {
        "v0.2.4 readiness freeze is absent",
        "Separate v0.2.4 execution freeze is absent",
    }
    criteria = {
        ...
        "execution_gate_fail_closed": (
            ...
            and "Separate v0.2.4 execution freeze is absent" in gate["failures"]
    ...
    return {
        ...
        "operation": "v0.2.4_runner_zero_science_integrity_dry_run",
        "criteria": criteria,
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="pilot2-injection-runner-v024")
    parser.add_argument(
